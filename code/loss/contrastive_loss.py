...
class ContrastiveLoss():
    ...
    def __call__(self, pred_path, data1, data2, match):
        ...
        loss1 = (match*dist1+(1-match)*F.relu(self.tau-dist1)).mean().clamp(min=self.epsilon)
        loss2 = (match*dist2+(1-match)*F.relu(self.tau-dist2)).mean().clamp(min=self.epsilon)
